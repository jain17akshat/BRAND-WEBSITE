...
export const MobileBottomNav = ({
  ...
  const items = [
    { label: 'Home', onClick: onGoHome, icon: Home, active: isHomeActive },
    { label: 'Categories', onClick: onOpenCategories, icon: Grid2x2, active: isCategoriesActive },
  ...
  return (
    ...
    >
      <div className="flex items-center justify-around px-1 py-2">
        {items.map(({ label, icon: Icon, onClick, badge, active }) => (
          <button
            key={label}
            onClick={onClick}
            aria-label={label}
          ...
          >
            <div className="relative">
              <Icon className={`w-5 h-5 ${active ? 'text-[#B8860B]' : ''}`} />
              {badge != null && (
            ...
            <span className={`text-[9px] font-cinzel tracking-wide ${active ? 'text-[#B8860B] font-bold' : ''}`}>
              {label}
            </span>
          </button>
        ))}
